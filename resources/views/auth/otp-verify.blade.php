...
@section('content')
    ...
    @if (session('status'))
        <div
            class="mb-5 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-300">
            {{ session('status') }}
        </div>
    @endif

    <form method="POST" action="{{ route('otp.verify.post') }}" x-data="{ loading: false }" @submit="loading = true"
        ...
        <div>
            ...
            <input id="email" type="email" name="email" value="{{ session('otp_email', old('email')) }}" required
                autocomplete="email"
                class="w-full rounded-xl border border-border-subtle bg-surface py-3 px-3 text-sm text-text-primary transition focus:border-gold focus:outline-none focus:ring-2 focus:ring-gold/20 dark:border-border-subtle-dark dark:bg-surface-dark dark:text-text-primary-dark @error('email') border-red-400 focus:ring-red-200 dark:border-red-500/60 dark:focus:ring-red-500/20 @enderror">
            @error('email')
    ...
    <div class="mt-6 text-center text-sm text-text-secondary dark:text-text-secondary-dark">
        ...
        <a href="{{ route('register') }}"
            class="ml-1 font-semibold text-gold-dark hover:text-gold dark:text-gold dark:hover:text-gold/80">Try again</a>
    </div>
@endsection
